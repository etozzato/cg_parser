module SearchIntegrateHelper
  ...
  def search_integrate_init(param_name='q')
    unless params[param_name.to_sym].blank?
      stylesheet_link_tag('searchintegrate') + "\n" + %(<div id='search_integrate' style='display:none;'></div>)
    end 
  end

  def search_integrate_exec(param_name='q')
    unless params[param_name.to_sym].blank?
      javascript_include_tag(wp_server + "/search.js?q=#{params[param_name.to_sym]}&limit=3") +
      "\n" +
      javascript_tag("if (typeof(search_integrate_content) != 'undefined'){ $('search_integrate').update(search_integrate_content);  $('search_integrate').appear()}")
    end 
  end
